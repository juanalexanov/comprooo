...
<head>
    ...
    <title>Lumina Admin - Edit About Us</title>
...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-1 text-gray-800">Edit About Us</h1>
                        ...
                        <div class="container mt-5">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="/admin/cms/aboutus/create" method="post" enctype="multipart/form-data">
                                        @csrf
                                        <div class="mb-3">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        <table class="table" id="table-Carousel">
                                            ...
                                            <tbody>
                                                @foreach ($dataaboutusjudul as $item)
                                                    <tr id = "{{$item->id}}">
                                                        <td>{{$loop->index+1}}</td>
                                                        <td class="text-center">{{ $item->title }}</td>
                                                        <td class="text-center">
                                                            <button class="btn btn-info btn-circle edit-judulaboutus"
                                                                    data-id="{{ $item->id }}"
                                            ...
                                            <tbody>
                                                @foreach ($dataaboutus as $item)
                                                    <tr id = "{{$item->id}}">
                                                        ...
                                                        <td class="text-center">{{ $item->title }}</td>
                                                        <td class="text-center">{{ $item->description1 }}</td>
                                                        <td class="text-center">{{ $item->description2 }}</td>
                                                        <td class="text-center">{{ $item->section1 }}</td>
                                                        <td class="text-center">{{ $item->section2 }}</td>
                                                        <td class="text-center">{{ $item->section3 }}</td>
                                                        <td class="text-center">
                                                            <button  class="btn btn-info btn-circle displayCarouselDetail">
                                                                <i class="fas fa-info-circle"></i>
                                                            </button>
                                                            <a href="/admin/cms/aboutus/delete/{{$item->id}}" class="btn btn-danger btn-circle">
                    ...
                    <div class="modal-content">
                        ...
                        <form action="" method="post" id = "form-update-displaybanner" enctype="multipart/form-data">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    <label for="Description1" class="form-label">Description 1</label>
                                    <input type="text" class="form-control" name="desc1modal" id="db-desc1">
                                </div>
                                <div class="mb-3">
                                    <label for="Description2" class="form-label">Description 2</label>
                                    <input type="text" class="form-control" name="desc2modal" id="db-desc2">
                                </div>
                                <div class="mb-3">
                                    <label for="Section1" class="form-label">Section 1</label>
                                    <input type="text" class="form-control" name="sec1modal" id="db-sec1">
                                </div>
                                <div class="mb-3">
                                    <label for="Section2" class="form-label">Section 2</label>
                                    <input type="text" class="form-control" name="sec2modal" id="db-sec2">
                                </div>
                                <div class="mb-3">
                                    <label for="Section3" class="form-label">Section 3</label>
                                    <input type="text" class="form-control" name="sec3modal" id="db-sec3">
                                </div>
                            </div>
            ...
            <div class="modal fade" id="updateJudulAboutusModal" tabindex="-1" aria-labelledby="updateJudulAboutusModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                <form action="/admin/cms/aboutus/judul/update/{id}" method="post" id="form-update-judulcard">
                    @csrf
                    <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="updateJudulAboutusModalLabel">Update Judul</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
    ...
    <script>
        ...
        $('#form-update-displaybanner').prop('action',"/admin/cms/aboutus/update/"+$(this).closest('tr').attr('id'));

        $('#db-title').val(temp.eq(2).html());
        $('#db-desc1').val(temp.eq(3).html());
        $('#db-desc2').val(temp.eq(4).html());
        $('#db-sec1').val(temp.eq(5).html());
        $('#db-sec2').val(temp.eq(6).html());
        $('#db-sec3').val(temp.eq(7).html());
        $('#modal-displayedit').modal('show');
    });
        $('.edit-judulaboutus').on('click', function() {
        // Dapatkan data dari atribut data-* yang telah Anda set pada tombol edit
        var judulaboutusId = $(this).data('id');
        var judul = $(this).data('judulmodal');

        // Isi form pada modal dengan data yang didapatkan
        $('#updateJudulAboutusModal').find('[name="judulmodal"]').val(judul);

        // Update action dari form pada modal
        var updateFormAction = '/admin/cms/aboutus/judul/update/' + judulaboutusId;
        $('#form-update-judulcard').attr('action', updateFormAction);

        // Tampilkan modal
        $('#updateJudulAboutusModal').modal('show');
    });
    </script>
</body>
